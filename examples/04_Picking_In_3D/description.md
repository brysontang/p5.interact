...
[dragging()](../../docs/reference/dragging.html) keeps `orbitControl()` from fighting a drag: the camera only orbits while nothing is held. [localMouse()](../../docs/reference/localMouse.html) and drag deltas are measured on the z = 0 plane of the current frame, so the box slides in the plane it lives in.
